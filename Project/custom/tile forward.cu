...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    float* first_shared = &shared_mem[0];
    ...
    int bx = blockIdx.x;
    int bz = blockIdx.z;
    int thread_idx = threadIdx.x + threadIdx.y * BLOCK_WIDTH;
    ...
    for(int i = 0; i < Channel; i++){
        ...
        if(bty < Height_out && btx < Width_out){
            first_shared[thread_idx] = in_4d(bz, i, bty, btx);
        }else{
            first_shared[thread_idx] = 0.0;
        }
        __syncthreads();
        ...
        if(bty < Height_out && btx < Width_out){
            for(int j = 0; j < K; j++){
                for(int k = 0; k < K; k++){
                    sum += in_4d(bz, i, bty + j, btx + k) * mask_4d(bx, i, j, k);
                }
            }
        }
        __syncthreads();
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    conv_forward_kernel<<<gridDim, blockDim, BLOCK_WIDTH * BLOCK_WIDTH * sizeof(float)>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
